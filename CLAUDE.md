...
- FOV（視界）
- セーブ/ロード（永続化・コンティニュー・スロット。現状は sessionStorage 復帰のみ）
- PWA対応（オフライン・ホーム画面追加）
- BGMトラック追加（暗黒城・深淵は未収録）
- 装備の強化・成長手段 / 特殊アイテムの挙動（謎の金庫・巻物） / 死亡ペナルティ・ダンジョン脱出
- 道具屋（アイテムの購入/売却。鍛冶屋・倉庫・村⇔ダンジョン遷移は実装済み）
- gameConfig 活用（ハードコード除去の完遂） / テスト拡充
