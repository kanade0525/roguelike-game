...
- デバッグモード（コナミコマンド起動・管理者用機能）
- 歩ける拠点（村）＋村長NPC（DungeonScene と同じ BaseMapScene 流用・鍛冶屋・ダンジョン選択・出口）
- 会話システム（DialogOverlay：話者名＋本文の逐次送り・日本語の折り返し対応）
- ストーリー/世界観（Katabasis＝冥界下り：オープニング／村長の進捗連動台詞／深淵踏破エンディング。`docs/story.md`・`game/story/`）
- クエストシステム（村長から受注→潜行で達成判定→報告→報酬。`game/quest/`・`meta.quests`）
- 専用オープニング（ゲーム画面内の黒幕カットシーン＋背景ピクセルアート。`pages/opening` は廃止し村内表示）
- タイル色分け（実加工タイル。村=灰白／静寂の森=緑／暗黒城=紫灰／深淵=赤黒。`terrainTextureSuffix`）
- ボス（森の主/城の主/深淵の主。大型スプライト・高ステータス・階段前配置・撃破必須・登場演出・専用ドロップ。`game/data/bossReward.ts`）
- 道具屋（購入/売却。`game/data/shop.ts`）＋拠点progression（静寂の森踏破で鍛冶屋・道具屋を解放）＋村の初期宝箱
- 村コマンドメニュー（道具/足もと/話す）＋持ち物確認、村長=老賢者スプライト
- 装備強化（鍛冶屋 enhanceEquipment）／死亡ペナルティ（半分ロスト）／ダンジョン脱出／謎の金庫／巻物（脱出=リレミト・ワープ・地図）
- 特殊アイテムの挙動（`game/systems/ItemSystem` の scrollAction を各層で処理）
- セーブ/ロード（中断ランを localStorage 保存し、ブラウザを閉じても「つづきから」で途中再開）
- FOV（視界）: rot-js PreciseShadowcasting による壁で遮る視界。既定 ON・`viewRange` 6（`game/systems/FOVSystem`・`recomputeFov`）
- 映画的タイトル画面（背景アート＋KATABASIS）
- コードレビュー実施＋修正（入力ロック参照カウント化・リスト/持ち物スクロール・getUiScene型統一 等）

### 未実装

- PWA対応（オフライン・ホーム画面追加）
- BGMトラック追加（暗黒城・深淵は未収録＝要音源ファイル）
- gameConfig 活用（ハードコード除去の完遂） / テスト拡充 / 全体バランス調整
