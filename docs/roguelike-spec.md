...
「Katabasis」— 不思議のダンジョン風ターン制ローグライク
...
短期リソース。満腹度が「探索を続けられる時間」を制限するのに対し、
スタミナは「連続で殴り続けること」を制限する。
...
- 攻撃力にそのまま乗る: `ダメージ = 攻撃力 × max(0.1, スタミナ/最大値)`
- 空になっても最低 10% の火力は残る（完全に無力化はしない）
- フロアをまたいでも持ち越す（回復手段は行動そのもの）
- 数値は `game/data/gameConfig.json` の `staminaConfig` に集約

#### 防御

- その場で身構える行動。1ターン消費する
- そのターンの被ダメージが 1/4 になる（防御力を引いた後に乗算、最低1）
- 同時にスタミナを最も多く回復する（+50）
- 次に行動した時点で解除される（1ターン限りの状態）
- 「殴り続ける」より「殴って引く」を有利にするための行動
